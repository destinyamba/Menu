...
def option4():
    ...
    next(record_file)
    next(record_file)
    contents_split = record_file.readlines()
    count = 0
    counter = 0
    for items in contents_split:
        lines = str(items.strip()).split(',')
        stocks = int(lines[5])
        count += stocks
        each_value = float(lines[6])
        counter += each_value
    print(f'New Stocks: #{count}')    # print new stock and value.
    print(f'New Value of Stocks: £{counter:,.2f}')
    record_file.close()

# This function allows the user to change the stock of a record.
def option5():
    print('Increse or Decrease stock: ')
    record_file = open('record_data.txt', 'r')
    ...
    while(file_read):
        ...
        if len(file_read) > 0:
            if lines[0] == search:
                ...
                temp_file.write(search + ', ' + lines[1] + ', ' + lines[2] + ', ' + lines[3] + ', ' + lines[4] + ', ' + new_stock + ', ' + lines[6])
                print(search + ', ' + lines[1] + ', ' + lines[2] + ', ' + lines[3] + ', ' + lines[4] + ', ' + new_stock + ', ' + lines[6])
            else:
